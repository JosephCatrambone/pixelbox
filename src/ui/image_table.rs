...
use eframe::{epi, egui::{self, Context, Ui, TextureId, ImageData, ColorImage, Image}};
use std::collections::HashMap;
use open;
use crate::ui::fetch_or_generate_thumbnail;

pub fn image_table(ui:&mut Ui, ctx: &Context, results:Vec<IndexedImage>, thumbnail_cache: &mut HashMap::<i64, TextureId>) {
	ui.vertical(|ui|{
		results.iter().for_each(|res|{
			ui.horizontal(|ui|{
				let tex_id = fetch_or_generate_thumbnail(res, thumbnail_cache, ctx);
